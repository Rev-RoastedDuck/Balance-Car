...
/*----------------------------------静态变量----------------------------------*/
/******************************************************************************/
static RRD_DRIVER_IIC g_OLED_IIC_DEVICE;


/******************************************************************************/
/*-----------------------------------适配层-----------------------------------*/
/******************************************************************************/
#define OLED_I2C_Stop()  						g_OLED_IIC_DEVICE.i2c_stop(&g_OLED_IIC_DEVICE)
#define OLED_I2C_Start() 						g_OLED_IIC_DEVICE.i2c_start(&g_OLED_IIC_DEVICE)
#define OLED_I2C_SendByte(byte)			OLED_I2C_SendByte_(&g_OLED_IIC_DEVICE,byte)
...
/**
  ...
  * @retval 无
  */
static void OLED_I2C_SendByte_(RRD_DRIVER_IIC *self,uint8_t Byte)
{
	g_OLED_IIC_DEVICE.i2c_send_byte(self,Byte);
	GPIO_WriteBit(OLED_IIC_PORT, OLED_SCL_PIN, (BitAction)(1));	//额外的一个时钟，不处理应答信号
	GPIO_WriteBit(OLED_IIC_PORT, OLED_SCL_PIN, (BitAction)(0));
}
...
static void OLED_I2C_Init(void)
{
	...
	iic_driver_initialize(&g_OLED_IIC_DEVICE);
	
	g_OLED_IIC_DEVICE.iic_scl_pin = OLED_SCL_PIN;
	g_OLED_IIC_DEVICE.iic_sda_pin = OLED_SDA_PIN;
	g_OLED_IIC_DEVICE.iic_gpio_port = OLED_IIC_PORT;
	g_OLED_IIC_DEVICE.i2c_gpio_config(&g_OLED_IIC_DEVICE);
}
...
void oled_show_float(uint8_t Line, uint8_t Column, float Number, uint8_t Precision) {
    ...
    oled_show_string(Line, Column, buffer);
}
...
/*-----------------------------------外部接口---------------------------------*/
/******************************************************************************/
RRD_DEVICE_OLED OLED_DRIVER = {
    .oled_init = oled_init,
    .oled_clear = oled_clear,
    .oled_show_char = oled_show_char,
    .oled_show_string = oled_show_string,
    .oled_show_num = oled_show_num,
    .oled_show_signed_num = oled_show_signed_num,
    .oled_show_hex_num = oled_show_hex_num,
    .oled_show_bin_num = oled_show_bin_num,
		.oled_show_float = oled_show_float
};
