...
#define i2c_write(addr,reg,len,buf)   MPU6050_DEVICE.mpu6050_write_data(reg,buf,len)
#define i2c_read(addr,reg,len,buf)    MPU6050_DEVICE.mpu6050_read_data(reg,buf,len)
#define delay_ms    									delay_ms_soft
...
/**
  * @brief   将陀螺仪方向矩阵转换为标量值
  * @param[in]   mtx   指向包含方向矩阵的数组的指针。矩阵应为 3x3 的矩阵，其中每行的 3 个元素表示一个方向。
  * @return      返回一个无符号短整型的标量值，表示方向矩阵的标量表示。
  * 
  * @details
  * 方向矩阵的排列方式如下：
  ...
  * - ZYX: 000_001_010
  * 
  * @note   此函数使用 `inv_row_2_scale` 函数将每行矩阵转换为标量，然后将这些标量合并成最终的标量值。
  * 
  * @retval 0 表示正常操作
  */
...
/**
  ...
  * 
  * @details
  * 此函数根据输入行的三个元素的值来确定标量值 `b`。标量值的计算规则如下：
  ...
  * 
  * @note   此函数用于将方向矩阵的每一行转换为一个标量值，通常用于进一步的处理或计算。
  * 
